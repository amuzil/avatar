...
import com.amuzil.caliber.physics.bullet.collision.space.supplier.level.LevelSupplier;
import com.amuzil.caliber.physics.bullet.thread.util.ClientUtil;
...
public class PhysicsThread extends Thread implements Executor {
    ...
    public PhysicsThread(Executor parentExecutor, Thread parentThread, LevelSupplier levelSupplier, EntitySupplier entitySupplier, String name) {
        ...
        CaliberPhysics.LOGGER.info("Starting " + getName());
        this.start();
    }
    ...
    public void destroy() {
        this.running = false;
        CaliberPhysics.LOGGER.info("Stopping " + this.getName());

        try {
            this.join(5000); // 5 second timeout
        }
        catch (InterruptedException e) {
            CaliberPhysics.LOGGER.error("Error joining " + this.getName());
            e.printStackTrace();
        }
    }
}
